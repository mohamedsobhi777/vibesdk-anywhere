...
export function getTemplatesData(): SecretTemplate[] {
	const templates = [
		...
		},
		{
			...
			instructions: 'Go to Z.ai Platform → API Keys → Create new key',
			placeholder: 'your-zai-key',
			validation: '^.{10,}$',
			required: false,
			category: 'ai',
		},
		...
		{
			id: 'ZAI_API_KEY_BYOK',
			displayName: 'Z.ai (BYOK)',
			envVarName: 'ZAI_API_KEY_BYOK',
			provider: 'zai',
			icon: '⚡',
			description: 'Use your Z.ai API key for GLM models',
			instructions: 'Go to Z.ai Platform → API Keys → Create new key',
			placeholder: 'your-zai-key',
			validation: '^.{10,}$',
			required: false,
